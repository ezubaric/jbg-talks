...
\begin{block}{Bai, et al.: Training a Helpful and harmless Assistant (2022)}
...
A question answering system is helpful if it provides responses that enable users to \alert<2>{maximally achieve their goals}
\end{block}
...
Check the model’s knowledge, ability to help users, \dots
	...
	\only<4>{\gfxm{llm_zoom_2}{1.0}}
	\only<5>{\gfxm{llm_zoom_3}{1.0}}
...
\begin{frame}{Outline for Today}
	...
	\begin{tabular}{ccc}
		\textbf{Task} & \textbf{Metric} & \textbf{Teaming} \\
		\toprule
		\alert<2>{Vocab Learning} & Time & Showing Mnemonics \\
		Fact Checking & Accuracy & Free Text Explanations \\
		Question Answering & Accuracy & Explanation Subsets \\
		Negotiation & Game Score & Computer Suggested Actions \\
		\bottomrule
	\end{tabular}
...
\begin{frame}{}
	\begin{columns}
		\column{.5\linewidth}
		\includegraphics[width=1.0\linewidth]{general_figures/nishant}
		\column{.5\linewidth}
		\gfxm{karl_paper}{1.0}
		\gfxm{mnemonic_paper}{1.0}
	\end{columns}
\end{frame}
...
\only<1>{\gfxm{compliant_0}{1.0}}
\only<2>{\gfxm{compliant_1}{1.0}}
\only<3->{\gfxm{compliant_2}{1.0}}
\only<4>{\gfxm{compliant_correct}{1.0}}
...
\begin{frame}{Objective Preferences: What \textbf{helps} users}
	
	\begin{columns}
		\column{.5\linewidth}
			\only<1>{\gfxm{objective_0}{1.0}}
			\only<2>{\gfxm{objective_1}{1.0}}
		\column{.5\linewidth}
			\begin{itemize}
				\item Ideal: randomized control w/ \abr{gre} scores
				\item Compromise: how many sessions until student retains word
				\item Assumption: more helpful mnemonics require fewer exposures
			\end{itemize}
	\end{columns}
...
\begin{frame}{SMART Mnemonics}
	\gfxm{smart_system}{1.0}
\end{frame}

\begin{frame}{How Good are the Mnemonics?}

\only<1>{\gfxm{eval_0}{0.7}}
\only<2>{\gfxm{eval_1}{0.7}}
\only<3->{\gfxm{eval_2}{0.7}}
...
\begin{frame}{Outline for Today}
	...
	\begin{tabular}{ccc}
		\textbf{Task} & \textbf{Metric} & \textbf{Teaming} \\
		\toprule
		Vocab Learning & Time & Showing Mnemonics \\
		\alert<1>{Fact Checking} & Accuracy & Free Text Explanations \\
		Question Answering & Accuracy & Explanation Subsets \\
		Negotiation & Game Score & Computer Suggested Actions \\
		\bottomrule
	\end{tabular}
...
\begin{frame}{FEVER categories}
	...
	(Poliak, 2018).  Actual claims come from Fool Me Twice (Eisenschlos, 2021).
\end{frame}
...
\begin{frame}{Remaining Issues}
	...
	\begin{itemize}
		\item If you could know if LLM was correct, problem becomes much simpler
		\item Calibration remains difficult
		\item Users are too trusting
		\item One size fits all: helpfulness depends on user skill
	\end{itemize}
...
\begin{frame}{Outline for Today}
	...
	\begin{tabular}{ccc}
		...
		Vocab Learning & Time & Showing Mnemonics \\
		Fact Checking & Accuracy & Free Text Explanations \\
		\alert<1>{Question Answering} & Accuracy & Explanation Subsets \\
		Negotiation & Game Score & Computer Suggested Actions \\
		\bottomrule
	\end{tabular}
...
\begin{frame}{Skill Boost}
  See how much interface boosts (or diminishes) user skill.
...
\begin{frame}{What is helpful?}
	\begin{itemize}
		\item Respecting users skill
		\item Sometimes the computer should keep quiet
		\item What is helpful to one user is not helpful to another
	\end{itemize}
\end{frame}
...
\begin{frame}{Outline for Today}
	...
	\begin{tabular}{ccc}
		...
		Fact Checking & Accuracy & Free Text Explanations \\
		Question Answering & Accuracy & Explanation Subsets \\
		\alert<1>{Negotiation} & Game Score & Computer Suggested Actions \\
		\bottomrule
	\end{tabular}
...
\begin{frame}{Outline for Today}
	...
	\begin{block}{Conclusion}
		\begin{itemize}
			\item Caveats and omissions: one dimensional skill
			\item Short-term next steps: helpful generation
		\end{itemize}
	...
	\begin{columns}
		...
		\only<1->{
			\begin{block}{\only<3->{Arnolfini Wedding}}
				...
				her suitor.
		\end{block}}
		\only<1->{\gfxa{caimira_abductive_skills}{1.0}}
	\end{columns}
	\only<1->{\vspace{-3.5cm}{\gfxa{ken_vs_hal_narrative}{.8}}}
\end{frame}
	...
	\begin{columns}
		...
		\only<1->{
			\begin{block}{\only<3->{Electron}}
				...
				state.
		\end{block}}
		\only<1->{\gfxa{caimira_science_skills}{1.0}}
	\end{columns}
	\only<1->{\vspace{-3.5cm}{\gfxa{ken_vs_hal_recall}{.8}}}
\end{frame}
...
\begin{frame}{Future Work}
	\begin{itemize}
		\item Workflows: Getting the right answer is not only objective (also trust and attribution)
		\item Agents: should test helpfulness at every step (plan generation and execution)
		\item Helpfulness in Decoding: Actually tailor the generation 
		\item Two-way calibration: Models should better estimate and communicate their confidence, and models should be able to better identify human confidence 
		\begin{itemize}
			\item Prosody
			\item Hedging
			\item False presuppositions
		\end{itemize}
	\end{itemize}
...
\begin{frame}{Recap}
	\includegraphics[width=1.0\linewidth]{general_figures/blackbox}
	\pause
	\vspace{-5cm}
	\begin{block}{What is the future of AI Collaboration?}
		\begin{itemize}
			\item Measure the ``skill'' of humans and computers
			\item Trust is hard to come by, and easily lost			
			\item Calibration and Minimizing Hallucinations
		\end{itemize}
	\end{block}
\end{frame}
...
\frame{
   ...
   \begin{frame}{Fool Me Twice}
   	...
   	\pause
   	\vspace{-5cm}
   	\begin{block}{Meant to be harder than FEVER}
