...
\begin{center}
  \includegraphics[width=.7\paperwidth]{svm/no-spam}
\end{center}
...
\begin{frame}{Is this a good Classifier?}
  ...
  \only<1>{\gfxi{lime_wolf}{.8}}
  \only<2>{\gfxi{lime_wolf_trust}{.8}}

\end{frame}

\begin{frame}{Improving ML Algorithms}

  \only<1>{\gfxi{lime_fe_overview}{.9}}
  \only<2>{\gfxi{mt_task}{.9}}
  \only<3>{\gfxi{mt_results}{.9}}
\end{frame}
