...
\ifjobtalk
\only<2->{ 
  Trivia: First UMD student, best paper KDD 2016
}
\fi
...
\begin{center}
\only<1>{When you at the dark side {\bf look}, careful you must {\bf
    be}. }
\only<2>{
...
% 大部分人是不耐烦。如果要等翻译，谈话很困难。
% Dà bùfèn rén shì bù nàifán. Rúguǒ yào děng fānyì, tánhuà hěn kùnnán.
...
\fsi{qb/nips_demo}{Can beat most ML researchers \ifjobtalk (NIPS 2015 Best Demo) \fi}
...
\begin{columns}
  ...
  \column{.5\linewidth}
    ...
    \centering
        \includegraphics[width=0.9\linewidth]{general_figures/tibp} \\
     \small
       \cite{hu-12b}
    \end{block}
...
\begin{frame}{Adding Meaning to Topic Models}
  ...
  \begin{itemize}
    \item Nested Dirichlet
      distributions~\cite{boyd-graber-07,andrzejewski-09}
    \item Retains conjugacy, unlike logistic normal
    \item Future direction: \alert<2>{co-occurence analysis}
    \end{itemize}
