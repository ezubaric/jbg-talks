...
\newcommand{\tb}[1]{\parbox{0.8\linewidth}{ \tiny{ #1 }} \vspace{.2cm} }
...
\frametitle{Simulating an interactive user}

	\begin{itemize}
		\item Take the 20 Newsgroups
		\item Imagine that we have a user that is reconstructing the newsgroups from undifferentiated data
			\begin{itemize}
				\item In practice, extract words from training data (information gain)
				\item {\bf soc.religion.christian}:  ``catholic, scripture, resurrection, pope, sabbath, spiritual, pray, divine, doctrine, orthodox''
			\end{itemize}
		\item Add them to the topic model as a positive correlation
		\item Train classifier on topic model document features
		\pause
		\item If test accuracy goes up, model is doing a better job of representing corpus
	\end{itemize}
...
\begin{block}{How much do you forget?}
  \begin{itemize}
    \item Tricky
    \item Need to control for amount of inference
   \item Forgetting document seemed to work best
     \item Details in paper
  \end{itemize}
\end{block}
...
\begin{center}
\includegraphics[width=\linewidth]{interactive_topic_models/ablation_30_topics.pdf}
\end{center}
