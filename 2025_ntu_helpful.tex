...
\newcommand{\gfxp}[2]{
\begin{center}
	\includegraphics[width=#2\linewidth]{planorama/#1}
\end{center}
}
...
\begin{frame}{Why do we use AI? (Ipsos)}

	\gfxm{gen_ai_uses}{1.0}
\end{frame}
...
\begin{frame}{Outline for Today}
	...
	\begin{itemize}
		...
		\item Helping users with plans (time + accuracy)
...
\begin{block}{Trace A ($n=3$)}
\end{block}

\begin{columns}
	\column{.18\linewidth}
	\gfxm{wrong_check}{1.0}
	\column{.18\linewidth}
	\gfxm{wrong_check}{1.0}
	\column{.18\linewidth}
	\gfxm{green_check}{1.0}
	\column{.18\linewidth}
	\column{.18\linewidth}
\end{columns}
...
Thus, we say that Trace A was more helpful.
...
\begin{frame}{Helpfulness as Finding a Plan}
	\begin{columns}
		\column{.5\linewidth}
			\includegraphics[width=.9\linewidth]{general_figures/nishant}
		\column{.5\linewidth}
			\begin{block}{A Good Plan is Hard to Find:
Aligning LLMs with Preferences is Misaligned with What Helps Users}
Nishant Balepur, Matthew Shu, Yoo Yeon Sung, Seraphina Goldfarb-Tarrant, Shi Feng, Fumeng Yang, Rachel Rudinger, and Jordan Boyd-Graber.  Empirical Methods in Natural Language Processing, 2025.
\end{block}
	\end{columns}
\end{frame}

\begin{frame}{How can Models Help Answer a Question?}

\only<1>{\gfxp{plan_intro_0}{1.0}}
\only<2->{\gfxp{plan_intro_1}{1.0}}

\only<3->{
\begin{block}{Benefits}
\begin{itemize}
%TODO: Add cites
\item Guidance (Bansal et al., 2021): AI-assisted decision-making
\item Scaffolding (Wood et al., 1976): Study aid for similar future questions
\end{itemize}
...
\begin{frame}{What's a Good Plan?}

\only<1->{\gfxp{better_plan_top}{1.0}}

\only<2-3>{\gfxp{better_plan_mid_0}{1.0}}
\only<4>{\gfxp{better_plan_mid_1}{1.0}}


\only<3->{\gfxp{better_plan_below}{1.0}}

\end{frame}

\begin{frame}{How much did the plan help?}

\only<1->{\gfxp{IRT_motivation_0}{1.0}}

\only<2->{\gfxp{IRT_motivation_1}{1.0}}

We can’t just use accuracy/speed without considering human abilities


%TODO: replace with images of Burt from Celebrity Jeopardy
\end{frame}
...
\begin{frame}{Measuring Helpfulness}

\begin{block}{IRT Equation}
\begin{equation}
	p(c) = \frac{1}{1 + \ex{-\left( \theta_s - \beta_q\only<2->{\alert<2>{ + \delta_p}}}\right)}
\end{equation}
\end{block}
...
\begin{itemize}
	\item IRT is essentially a regression, fit on people answering questions
	\item $\delta_p$ is boost to skill / decrease difficulty
	\item Complication: incorporating time
\end{itemize}
...
\begin{frame}{Results}
	...
	\only<1>{\gfxp{results_0}{1.0}}
	\only<2>{\gfxp{results_1}{1.0}}
	\only<3>{\gfxp{results_2}{1.0}}
	\only<4>{\gfxp{results_3}{1.0}}
	\only<5>{\gfxp{results_4}{1.0}}
	\only<6>{\gfxp{results_5}{1.0}}

\only<4>{Agreement matrix of preferences and helpfulness across humans and models}

\only<5>{Nothing predicts what helps humans}

\only<6>{Reward models + GPT don’t even know what helps GPT!}
\end{frame}
...
%TODO: Conclusion should do a better job of saying:
% I didn't actually tell you how to help people
% we still need to figure that out
% larger scale data, more tasks
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Unreliable 
				\item Sychophantic
				\item \textbf{Neither LLMs nor humans can identify what will help}
			\end{itemize}
...
\pause
		\item Creating outputs that are actually helpful
	\end{itemize}
\end{frame}
...
\begin{frame}{Adversarial Strategies}
	\begin{columns}
		\column{.5\linewidth}
		\only<1-3>{\gfxa{akutagawa}{.9}}
		\only<1-3>{A character in one story by \alert<2>{this author opens Crime and Punishment} to discover that it has turned into The Brothers Karamazov}
		\only<3>{\\ \textbf{Dostoyevski / Akutagawa}}
	\end{columns}
\end{frame}
...
\begin{frame}{Claim: Manhattan has the most residents of New York City's boroughs. }
	\begin{columns}
		...
		\only<3->{
			\begin{block}{Direct}
				\scriptsize The claim that Manhattan has the most residents of New York City's boroughs \alert<4>{is false}.  Manhattan has a population of approximately 1.6 million residents. However, New York City as a whole has a population of approximately 8.3 million residents. Therefore, Manhattan does not have the most residents among New York City's boroughs.  
			\end{block}
		}
		\only<4->{
