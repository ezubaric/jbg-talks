...
\only<3>{\gfxa{jeopardy_irt_0}{.9}}
\only<4>{\gfxa{jeopardy_irt_1}{.9}}
\only<5>{\gfxa{jeopardy_irt_2}{.9}}
...
\end{frame}
 ...
 \end{frame}
  ...
  \begin{frame}{Adversarial Datasets}
    ...
    \begin{columns}
      \column{.5\linewidth}
%TODO(jbg): DADC saturation figure
            \column{.5\linewidth}
            \begin{itemize}
 ...
 \end{frame}
    ...
    \begin{frame}{Expanding IRT: Discriminability}
      ...
      % TODO(jbg): Add perfect discriminability pictures
...
\end{itemize}
    ...
    \begin{frame}{Is this a viable incentive structure?}
      \begin{itemize}
...
\begin{frame}{Can they tell it's a bot?}
	\begin{columns}
		...
		\end{block}
		\column{.5\linewidth}
		\only<1>{\gfx{benchmark_detection_0}{.9}}
		\only<2>{\gfx{benchmark_detection_1}{.9}}
		\only<3>{\gfx{benchmark_detection_2}{.9}}
		\only<4>{\gfx{benchmark_detection_3}{.9}}
	\end{columns}
\end{frame}

\begin{frame}{Persuasion}
	\only<1>{\gfxd{persuasion}{.2}}
...
\begin{frame}{Broken Commitments (subset of deception)}
	...
	\only<2>{\gfxd{cicero_deception_2}{.5}}
	\only<3>{\gfxd{cicero_deception_3}{.75}}
\only<4>{\gfxd{cicero_deception_4}{1.0}}	
\end{frame}
...
%TODO(jbg): Add Feng slide


\begin{frame}{Giving Advice}
	\only<1>{\gfx{pholus_inteface}{1.0}}
	\only<2>{\gfx{pholus_interface_label}{1.0}}
\end{frame}

\begin{frame}{Advice Helps}
	\only<1>{\gfxd{chiron_skill_regression}{.5}}
...
\fsi{general_figures/robot_shake}{We need ML that understands our gratitude and our fears}
