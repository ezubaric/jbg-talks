...
\begin{itemize}
  \item \only<2->{ If you have to wait a long time for a translation,
      it makes a real conversation 
    \emph{impossible}!}

   \only<3->{\item Much better \only<4->{if you could get}
     \only<5->{translations as soon as possible}}

   \only<6->{\item But machine translation (mostly) works the same way}
...
\begin{frame}{Common Foundation}

\begin{columns}

  \column{.5\linewidth}
  \begin{itemize}
    \item We have a set of \alert<2>{actions} (answer / translate)
